<%@ page title="Benchmarks-RegionToCountry" language="C#" masterpagefile="~/CRM.master" autoeventwireup="true" inherits="Reports_Benchmarks_RegionToCountry, App_Web_1tplfa14" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="Server">
    <div id="dialog" title="Report: Franchisee to Region">
        <table>
            <tr>
                <td>
                    <asp:DropDownList ID="regionList" runat="server" Visible="false">
                    </asp:DropDownList>
                </td>
            </tr>
            <tr>
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            var regionList = $('#<%=regionList.ClientID %>');
            ...
            $('#btnGetReport').click(function () {
                if (regionList.val() != '') {
                    ...
                    $('#dialog').dialog('close');
                    chartContainer.style.display = "block";
                    loadChart(chartIds[0], '', regionList.val(), 'chartContainer', 'myChartId');
                }
            });

            function loadChart(chartId, drillBy, regionId, chartContainer, myChartId) {
                $.ajax({
                    url: baseUrl + "/api/Chart/",
                    type: 'GET',
                    contentType: 'application/json',
                    data: { id: '', strChartIds: chartId, strChartSubType: chartSubType, strDrillBy: drillBy, strUserName: userName, strSearchParameter: regionId },
